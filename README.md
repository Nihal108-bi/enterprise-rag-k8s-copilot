...
**EnterpriseRAG Live** is a production-oriented GenAI backend project for Kubernetes IT-Operations. It demonstrates how to build a real enterprise assistant that can answer both unstructured documentation questions and structured operational analytics questions. The system is not a simple prompt wrapper. It includes authentication, rate limiting, token budgets, advanced retrieval, Text2SQL, human-in-the-loop SQL approval, Redis caching, prompt-injection defenses, PII redaction, document ingestion, evaluation, and containerized deployment.

**Business use case:** An SRE or platform engineer asks questions such as:

- "How does a Kubernetes Deployment handle rolling updates?"
- "Which cluster had the most P1 incidents last month?"
- "Show P1 incidents for prod-us-east and explain remediation steps."

The system routes each question to the correct execution path:

| User question type | Route | Why |
|---|---|---|
| Kubernetes concepts, runbooks, troubleshooting | RAG | Answer requires unstructured documentation |
| Incident counts, MTTR, alert statistics, cluster facts | Text2SQL | Answer requires structured operational data |
| Incident facts plus remediation guidance | Hybrid | Answer needs both database rows and retrieved documents |

**Interview positioning:** This project shows applied AI engineering across backend APIs, retrieval systems, agent orchestration, SQL safety, security guardrails, caching, evaluation, and deployment. It is suitable for AI Application Engineer, GenAI Engineer, LLM Engineer, Backend AI Engineer, and Applied AI roles.

## Downloadable Diagram Assets

All Mermaid diagrams are available as standalone `.mmd` files in [`docs/diagrams`](docs/diagrams). Each file includes a diagram title and has a recommended image filename, so exported images have clear names such as `system-design-architecture.png`, `code-flow-architecture.png`, and `evaluation-architecture.png`.

| Diagram | Mermaid Source | Recommended Downloaded Image Name |
|---|---|---|
...
Export one diagram as an image:
...
Export all diagrams with named image files:
...
This diagram shows how the codebase is organized from API entrypoints to LangGraph orchestration, services, storage, and evaluation. It is useful during interviews because it proves the project has backend structure, not just notebook-level experimentation.
...
[Download Mermaid source: document-ingestion-architecture.mmd](docs/diagrams/11_document_ingestion_architecture.mmd)
...
The evaluation layer is designed to compare retrieval configurations rather than only test whether the API returns a response. The harness loads golden questions from `eval/seed_questions.yaml`, runs a selected profile from `eval/profiles.py`, captures answers and retrieved contexts, scores them with Ragas, then applies custom checks for source overlap and forbidden keywords.
...
flowchart TD
    ...
    Aggregate --> Results["eval/results/*.json"]
    Results --> Streamlit["Streamlit Evaluation Dashboard"]
```

Interview-ready explanation:

> I evaluate the system at the retrieval and answer level. Ragas measures faithfulness, context precision, context recall, and answer relevancy. My custom checks verify whether expected sources were retrieved and whether forbidden terms appeared in the answer. This gives me separate visibility into retrieval quality, generation quality, and safety.
...
- Built an enterprise-grade Kubernetes IT-Ops copilot using FastAPI, LangGraph, Qdrant, PostgreSQL, Redis caching, OpenAI models, Text2SQL, and Ragas evaluation.
- Implemented advanced RAG with dense retrieval, sparse retrieval, Reciprocal Rank Fusion, HyDE, cross-encoder reranking, CRAG web fallback, Self-RAG reflection, and source-grounded generation.
- Designed a safe Text2SQL workflow with schema introspection, SELECT-only validation, SQL result caching, LangGraph checkpointing, and human-in-the-loop approval before query execution.
- Added production security layers including JWT authentication, rate limiting, token budgets, prompt-injection scanning, input restructuring, PII redaction, spotlighting, hardened system prompts, and output validation.
- Created an evaluation harness using Ragas metrics, source-overlap checks, forbidden-keyword checks, and profile-based comparisons across naive, hybrid, rerank, HyDE, CRAG, and all-feature pipelines.

### 45-Second Interview Explanation

> EnterpriseRAG Live is a production-oriented RAG and Text2SQL copilot for Kubernetes operations. The user asks a natural-language question through FastAPI or Streamlit. The backend authenticates the user with JWT, applies rate limits and token budgets, scans for prompt injection, redacts PII, and sends the request into a LangGraph state machine. The router decides whether the question needs RAG, SQL, or both. RAG queries use Qdrant, dense and sparse retrieval, RRF fusion, optional HyDE, reranking, CRAG fallback, and Self-RAG reflection. SQL questions use schema introspection, LLM-generated SELECT queries, human approval through LangGraph interrupts, and cached SQL results. The final answer is moderated, validated, returned with sources and metadata, and evaluated using Ragas plus custom source and safety checks.

### Production Engineering Signals

| Signal | Where It Appears |
|---|---|
| Secure API design | JWT auth, rate limiting, token budgets, PII redaction |
| Real retrieval engineering | Qdrant, hybrid search, RRF, reranking, HyDE, CRAG |
| Agent orchestration | LangGraph routing, checkpointing, interrupt/resume |
| Structured-data reasoning | Text2SQL with schema introspection and SELECT-only validation |
| Cost control | Redis caches for intent, embeddings, RAG answers, SQL generation, SQL results |
| Evaluation maturity | Ragas metrics, source overlap, forbidden keyword checks, eval profiles |
| Deployment readiness | Dockerfile, Docker Compose, environment config, AWS production path |
